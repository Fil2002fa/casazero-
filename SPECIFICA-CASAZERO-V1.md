...
1. **M1 — Fondamenta**: progetto Next.js + Supabase, schema DB completo con RLS,
   seed delle 27 manutenzioni template, auth (Google + magic link), layout PWA con palette C.
2. **M2 — Motore manutenzioni**: calcolo scadenze, cron notifiche, regole N1/N2/N3,
   flusso completamento N2 con allegati, ciclo N3 amministratore.
3. **M3 — Fascicolo e documenti**: timeline completions, conformità %, upload/download
   documenti per categoria, commenti per manutenzione.
4. **M4 — Onboarding e admin**: inviti QR, wizard date edifici esistenti, dashboard
   essenziale super admin, white-label.
5. **M5 — Rifinitura**: report PDF, notifiche push, preferenze profilo, seed demo
